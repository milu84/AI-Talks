...
def generate_response(
    ...
    Generates a response from a specific LLM, based on:
      - `speaker`: which model we’re calling (e.g., "OpenAI-GPT", "Gemini", "Cloud", "Grok", "DeepSeek")
    ...
    elif "deepika" in speaker_lower:
        response_text = call_deepseek_api(user_prompt, api_key, speaker, version)
    else:
...
def call_deepseek_api(prompt_text: str, api_key: str, name: str, version: str) -> str:
    """ Call to DeepSeek’s Chat API using the OpenAI client with a custom base URL. """
    if not api_key:
        return f"Error: Missing API key for {name}."
    ...
    try:
        response = client.chat.completions.create(
            model=version,  # Typically "deepseek-chat"
            messages=[
                {"role": "system", "content": "You are a helpful assistant."},
                {"role": "user", "content": prompt_text}
            ],
...
def main():
    ...
    # Provide defaults if config is empty, now including a DeepSeek model.
    if not models_list:
        models_list = [
            ...
            {'name': 'DeepSeek', 'apikey': None, 'version': 'deepseek-xx'}
        ]
