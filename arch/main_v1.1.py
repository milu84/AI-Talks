...
def sanitize_llm_output(response_text: str, speaker: str) -> str:
    ...
    pattern = r'^(?:\[' + re.escape(speaker) + r'\]:\s*)?(?:\[' + re.escape(speaker) + r'\]:\s*)?(?:' + re.escape(speaker) + r':\s*)?(.*)$'
...
def generate_response(
    ...
    if ext_data:
        external_data_string = f"***** Here is additional, external data (use it only if relevant, and not in all your answers) ***** \n\n: {ext_data}\n\n"
    else:
        external_data_string = ""

    # **Key addition**: We explicitly instruct the model to NOT include its own name or prefix.
    # We will add [Speaker]: ourselves after post-processing.
    user_prompt = (
        f"{style_prompt}\n\n"
...
def main():
    ...
    for model in models_list:
        ...
        # model_intro = f"- {model['name']} (model: {model['version']})"
    ...
    # final_round_marker = f"***** FINAL ROUND (Total so far: {total_characters} characters) *****"
